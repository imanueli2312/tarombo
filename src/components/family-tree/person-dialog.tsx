...
export function PersonDialog({
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast.success(isExisting ? "Person updated." : "Person created.");
      onSaved?.();
      onOpenChange(false);
    } catch (e: any) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3 pr-8">
            ...
            {isDeceased && (
              <Badge variant="secondary" className="gap-1">
                <Cross className="h-3 w-3" /> Almarhum
              </Badge>
            )}
          ...
          <DialogDescription>
            {person.nickname ? `Nickname: "${person.nickname}"` : "No nickname set."}
          </DialogDescription>
        </DialogHeader>
          ...
          <TabsContent value="detail" className="space-y-4 pt-2">
            <div className="flex gap-4">
              ...
              <div className="grid flex-1 grid-cols-2 gap-x-4 gap-y-2 text-sm">
                <InfoRow icon={<Calendar />} label="Born">
                  {person.place_of_birth ? `${person.place_of_birth}, ` : ""}
                  {fmt(person.date_of_birth) || "—"}
                </InfoRow>
                {isDeceased && (
                  <InfoRow icon={<Cross />} label="Died">
                    {fmt(person.date_of_death)}
                  </InfoRow>
                )}
                <InfoRow icon={<Heart />} label="Marital">
                  <span className="capitalize">{person.marital_status || "—"}</span>
                </InfoRow>
                <InfoRow icon={<User />} label="Birth order">
                  {person.birth_order || "—"}
                </InfoRow>
                ...
                {person.religion && (
                  <InfoRow icon={<User />} label="Religion">
                    {person.religion}
                  </InfoRow>
                )}
              </div>
            ...
            <div className="rounded-lg border bg-muted/30 p-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                  <span className="text-muted-foreground">Official parent: </span>
              ...
              {((person as TreeNode).children?.length ?? 0) > 0 && (
                <div className="mt-2 text-sm">
                  <span className="text-muted-foreground">Children ({(person as TreeNode).children!.length}): </span>
            ...
            <TabsContent value="edit" className="space-y-4 pt-2">
                ...
                <div className="space-y-1.5">
                  ...
                  {form.photo && (
                    <div className="mt-1 flex items-center gap-2 text-xs">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => update("photo", null)}>Remove</Button>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-lg border bg-muted/30 p-3">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  ...
                  <div className="space-y-1.5">
                    <Label>Official parent (tree position)</Label>
