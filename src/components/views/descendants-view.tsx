...
import { Loader2, Users, Search as SearchIcon } from "lucide-react";
import { toast } from "sonner";
import { useLanguage } from "@/hooks/use-language";
import type { Person } from "@/lib/types";

interface DescendantPerson extends Person {
  depth: number;
}

export function DescendantsView() {
  const { t } = useLanguage();
  const [allPersons, setAllPersons] = useState<Person[]>([]);
  const [descendants, setDescendants] = useState<DescendantPerson[]>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        toast.error(t("descendants.loadFailed"));
      } finally {
        setLoading(false);
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/descendants?personId=${selectedId}`);
        if (!res.ok) throw new Error();
        setDescendants(await res.json());
      } catch {
        toast.error(t("descendants.loadFailed"));
      }
    })();
    ...
    <div className="scroll-area-thin h-full overflow-auto">
      <div className="mx-auto max-w-3xl space-y-4 p-4 sm:p-6">
        <header>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{t("descendants.subtitle")}</p>
        </header>
          ...
          <CardContent className="p-4">
            <div className="relative mb-2">
              ...
              <Input
                ...
                placeholder={t("descendants.searchPlaceholder")}
                className="pl-9"
              />
            </div>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      Gen {a.generation} {a.depth > 0 ? `· ${a.depth === 1 ? "Child" : a.depth === 2 ? "Grandchild" : `+${a.depth} gen`}` : "· Self"}
                    </div>
                  </div>
                </button>
              </div>
